...
func _ready():
	...
	$AnimatedSprite2Df.play("Sidlef")  # Start with idle animation
...
func respawn() -> void:
	...
	$AnimatedSprite2Df.play("Sidlef")  # Reset animation
	...
	if player:
		...
		if abs(direction_to_player.x) > abs(direction_to_player.y):
			if direction_to_player.x > 0:
				$AnimatedSprite2Df.play("Sattackf")  # Attack to the right
				$AnimatedSprite2Df.flip_h = false
			else:
				$AnimatedSprite2Df.play("Sattackf")  # Attack to the left
				$AnimatedSprite2Df.flip_h = true
		else:
			if direction_to_player.y > 0:
				$AnimatedSprite2Df.play("Fattackf")  # Attack downwards
				$AnimatedSprite2Df.flip_h = false
			else:
				$AnimatedSprite2Df.play("Battackf")  # Attack upwards
...
func update_animations(direction: Vector2) -> void:
	var anim_sprite = $AnimatedSprite2Df
	...
	if direction.length() > 0:
		if direction.x > 0:
			anim_sprite.play("Swalkf")   # Right movement animation
			anim_sprite.flip_h = false
		elif direction.x < 0:
			anim_sprite.play("Bwalkf")  # Left movement animation
			anim_sprite.flip_h = true
		elif direction.y > 0:
			anim_sprite.play("Fwalkf")  # Down movement animation
			anim_sprite.flip_h = false
		elif direction.y < 0:
			anim_sprite.play("Bwalkf")  # Up movement animation
	else:
		anim_sprite.play("Sidlef")
