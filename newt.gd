...
@export var attack_delay = 1.5  # Increased the attack delay to slow down attacks
...
@export var attack_animation_time = 0.8  # Time it takes for the attack animation to complete
...
func _ready():
	...
	$AnimatedSprite2Dn.play("Sidlen")  # Start with idle animation
...
func respawn() -> void:
	...
	$AnimatedSprite2Dn.play("Sidlen")  # Reset animation
...
func chase_player(delta, distance_to_player):
	...
	if distance_to_player <= attack_range:
		...
		if not attacking:
			...
			if attack_timer <= 0:
				perform_attack()
				attack_timer = attack_delay  # Reset attack timer after the attack
	else:
	...
	if player:
		...
		if abs(direction_to_player.x) > abs(direction_to_player.y):
			if direction_to_player.x > 0:
				$AnimatedSprite2Dn.play("Sattackn")  # Attack to the right
				$AnimatedSprite2Dn.flip_h = false
			else:
				$AnimatedSprite2Dn.play("Sattackn")  # Attack to the left
				$AnimatedSprite2Dn.flip_h = true
		else:
			if direction_to_player.y > 0:
				$AnimatedSprite2Dn.play("Fattackn")  # Attack downwards
				$AnimatedSprite2Dn.flip_h = false
			else:
				$AnimatedSprite2Dn.play("Battackn")  # Attack upwards
		
		# Wait for the attack animation duration before inflicting damage
		await get_tree().create_timer(attack_animation_time).timeout  # Sync with the attack animation
...
func update_animations(direction: Vector2) -> void:
	var anim_sprite = $AnimatedSprite2Dn

	# Prevent overriding the attack animation if it's playing
	if anim_sprite.is_playing() and attacking:
		return

	if direction.length() > 0:
		if direction.x > 0:
			anim_sprite.play("Swalkn")   # Right movement animation
			anim_sprite.flip_h = false
		elif direction.x < 0:
			anim_sprite.play("Bwalkn")  # Left movement animation
			anim_sprite.flip_h = true
		elif direction.y > 0:
			anim_sprite.play("Fwalkn")  # Down movement animation
			anim_sprite.flip_h = false
		elif direction.y < 0:
			anim_sprite.play("Bwalkn")  # Up movement animation
	else:
		anim_sprite.play("Sidlen")
...
func _on_hitbox_area_2d_body_exited(body: Node2D) -> void:
	pass # Replace with function body.
